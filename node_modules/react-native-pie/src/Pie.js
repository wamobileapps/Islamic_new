...
const Sections = ({dimensions, paintedSections, sections, shouldShowRoundDividers, strokeCapForLargeBands}) => {
  ...
  sections == undefined ? null : paintedSections = sections.map((section, idx) => {
    const { percentage, color,text } = section;
    const startAngle = startValue / 100 * 360;
    const arcAngle = getArcAngle(percentage);
    startValue += percentage;
    shouldShowRoundDividers && paintedSections.push({ percentage, color, startAngle, arcAngle , text});
    return(
