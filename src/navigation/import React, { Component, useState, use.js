...
const changeTimeFormat = (timeString) => {
 ...
 console.log("time string data ", totalValFajeBegins, timeString, H, h, hr, min);
    ...
    axios.get(`https://www.londonprayertimes.com/api/times/?format=json&key=9e2a1077-7c3f-4a0a-a284-2f68a58b02cc&year=${global.calYear}&month=${global.calMonth}&24hours=false`)
      .then((response) => {
       ...
       var c = []
        ...
        console.log("london time===>", response.data.times);
        ...
        filterArr.map((i) => {
          ...
          axios.get('http://112.196.64.119:8000/api/user/manualprayerstime/list', {
            ...
            .then((response) => {
              ...
              convertSunTime(global.sun,response.data.data[0].type6.begins_time, response.data.data[0].type6.jamah_time, response.data.data[0].type6.alarm_time )
              }
  ...
  const getDataMonth=()=>{
    ...
    global.calYear = global.calYear ? global.calYear : moment().format('YYYY')
// alert(global.calYear)
